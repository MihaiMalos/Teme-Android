package com.example.listaanimale.data.tasks

import android.os.AsyncTask
import com.example.listaanimale.ApplicationController
import com.example.listaanimale.data.models.AnimalModel

class InsertAllAnimalsTask(
    private val onSuccess: () -> Unit,
) : AsyncTask<List<AnimalModel>, Unit, Unit>() {
    override fun doInBackground(vararg params: List<AnimalModel>) {

        params.getOrNull(0)?.let {animals ->
            ApplicationController.instance.appDatabase.animalsDao.insertAll(animals)
        }

    }

    override fun onPostExecute(result: Unit?) {
        onSuccess.invoke()
    }


}